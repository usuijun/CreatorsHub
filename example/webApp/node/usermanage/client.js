...
function init() {
  ...
  gSocket.on("usernumber", function(args) {
    console.log("My user number is " + args.number);
    userNumber = args.number;
    ...
    ctx.font='10px Arial';
    ctx.textAlign='left';
    ctx.fillText("User Number:"+userNumber, 10, ypos);
  });
...
function sendDown(x, y, id) {
  ...
  x = x / windowWidth;
  y = y / windowHeight;
  gSocket.emit("down", {x:x, y:y, id:id, ch:userNumber});
}
...
function sendUp(x, y, id) {
  ...
  x = x / windowWidth;
  y = y / windowHeight;
  gSocket.emit("up", {x:x, y:y, id:id, ch:userNumber});
}
